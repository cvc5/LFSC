...
enum
{
  NOT_CEXPR = 0,  // for INT_EXPR, HOLE_EXPR, SYM_EXPR, SYMS_EXPR
  APP,
  PI,
  LAM,
  TYPE,
...
class Expr
{
 protected:
  ...
  void debugrefcnt(int ref, int what)
  {
    ...
    switch (what)
    {
      case INC: std::cout << " inc to "; break;
      case DEC: std::cout << " dec to "; break;
      case CREATE: std::cout << " creating]\n"; return;
    }
    char tmp[10];
    sprintf(tmp, "%d", ref);
    std::cout << tmp << "]\n";
...
class CExpr : public Expr
{
 public:
  ...
  CExpr(int _op, Expr *e1, Expr *e2) : Expr(CEXPR, _op), kids()
  {
    kids = new Expr *[3];
    kids[0] = e1;
    kids[1] = e2;
  ...
  CExpr(int _op, Expr *e1, Expr *e2, Expr *e3) : Expr(CEXPR, _op), kids()
  {
    kids = new Expr *[4];
    kids[0] = e1;
    kids[1] = e2;
  ...
  CExpr(int _op, const std::vector<Expr *> &_kids) : Expr(CEXPR, _op), kids()
  {
    int i, iend = _kids.size();
    kids = new Expr *[iend + 1];
    for (i = 0; i < iend; i++) kids[i] = _kids[i];
    kids[i] = 0;
#ifdef DEBUG_REFCNT
    debugrefcnt(1, CREATE);
#endif
...
class SymExpr : public Expr
{
 public:
  ...
  SymExpr(std::string _s, int theclass = SYM_EXPR) : Expr(theclass, 0), val(0)
  {
    (void)_s;
#ifdef DEBUG_REFCNT
    if (theclass == SYM_EXPR) debugrefcnt(1, CREATE);
#endif
  }
  SymExpr(const SymExpr &e, int theclass = SYM_EXPR) : Expr(theclass, 0), val(0)
  {
    (void)e;
#ifdef DEBUG_REFCNT
    if (theclass == SYM_EXPR) debugrefcnt(1, CREATE);
...
class HoleExpr : public Expr
{
 ...
 public:
  ...
  int id;
#endif
  HoleExpr() : Expr(HOLE_EXPR, 0), val(0)
  {
#ifdef DEBUG_HOLE_NAMES
    id = next_id++;
#endif
...
inline Expr *Expr::followDefs()
{
  switch (getclass())
  {
    case HOLE_EXPR:
    {
      ...
      if (h->val) return h->val->followDefs();
      break;
    }
    case SYMS_EXPR:
    case SYM_EXPR:
