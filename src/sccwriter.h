...
class sccwriter
{
 private:
  ...
  void write_code(Expr* code,
                  std::ostream& os,
                  int ind,
                  const char* retModStr,
                  int opts = 0);
  // write all children starting at child counter to stream, store in Expr*
  // e_...e_;
  void write_args(CExpr* code,
                  std::ostream& os,
                  int ind,
                  int childCounter,
  ...
  void write_expr(
      Expr* code, std::ostream& os, int ind, std::string& expr, int opts = 0);
  // write variable
